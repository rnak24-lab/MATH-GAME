...
/// 미연시식 대화 상자 — 화면 아래(책상 자리)에 뜬다. 한 글자씩 타이핑되고,
/// 탭하면 다 보여주기 → 다음 줄. 마지막 줄에서 탭하면 [onDone].
/// 표정은 [onFace] 로 바깥(큰 예린)에 넘긴다 — 화면에 예린은 항상 한 명.
class DialogueBox extends StatefulWidget {
  final List<DialogueLine> lines;
  final String speaker;
  final String? title; // 장면 제목 (호감도 장면일 때)
  final void Function(MidnightFace face) onFace;
...
class _DialogueBoxState extends State<DialogueBox> {
  ...
  String get _text => widget.lines[_index].text;
  ...
  void _startLine() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) widget.onFace(widget.lines[_index].face);
    });

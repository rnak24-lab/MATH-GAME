...
///
/// 광고 완전 비활성 출시: [adsEnabled] = false.
/// 광고 재활성 시: [adsEnabled] = true + (필요 시) AndroidManifest에
/// `com.google.android.gms.ads.APPLICATION_ID` meta-data 재추가.
class AdIds {
  /// 전역 광고 활성화 스위치. 출시 RC(광고 비활성)는 false.
  /// 실제 AdMob ID 확정 시 true 로 토글하고 아래 _realBannerId/_realInterstitialId 교체.
...
///
/// [AdIds.adsEnabled] == false 인 경우 모든 경로가 no-op 이 된다.
class AdService {
  AdService._();
  ...
  Future<void> init() async {
    if (!AdIds.adsEnabled) {
      debugPrint('[AdService] adsEnabled=false → SDK 초기화 스킵');
      return;
    }
  ...
  /// 광고 비활성 시 null 반환 → BannerAdWidget이 빈 위젯 처리.
  BannerAd? createBannerAd({
    required VoidCallback onLoaded,
    VoidCallback? onFailed,
  }) {
    if (!AdIds.adsEnabled) return null;
    return BannerAd(
  ...
  bool maybeShowInterstitialOnStageClear() {
    if (!AdIds.adsEnabled) return false;
    _stageClearCount++;
